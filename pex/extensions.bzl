...
load(
    "//pex/private:pex_tools_repo.bzl",
    "CONSTRAINTS",
    "PEX_DEFAULT_VERSION",
    "PEX_VERSIONS",
    ...
    "pex_windows_fallback_tool_repository",
...
def _pex_toolchain_impl(module_ctx):
    ...
    # Collect which toolchains should have fallback Windows binaries
    windows_fallback_toolchains = {}
    for mod in module_ctx.modules:
        for attrs in mod.tags.add_fallback_windows_binaries:
            windows_fallback_toolchains[attrs.toolchain_name] = None

    windows_fallback_toolchains = list(windows_fallback_toolchains.keys())
    ...
    for mod in module_ctx.modules:
        for attrs in mod.tags.toolchain:
            ...
            # Determine which Windows platforms to add as fallbacks
            windows_fallback_platforms = []
            if attrs.name in windows_fallback_toolchains:
                all_windows_platforms = [p for p in CONSTRAINTS.keys() if p.startswith("windows")]
                for windows_platform in all_windows_platforms:
                    # Only add if not already in the available platforms
                    if windows_platform not in scie_platforms and windows_platform not in python_platforms:
                        windows_fallback_platforms.append(windows_platform)

                        # Create a single fallback repository for this platform
                        # All tools (pex, science, jump, ptex, python) will reference this
                        fallback_repo = "{}_fallback_{}".format(attrs.name, windows_platform)
                        pex_windows_fallback_tool_repository(name = fallback_repo)

            # Add Windows fallback platforms to our platform lists
            scie_platforms = scie_platforms + windows_fallback_platforms
            python_platforms = python_platforms + windows_fallback_platforms
            ...
            for python_platform in python_platforms:
                ...
                elif python_platform in windows_fallback_platforms:
                    # Use fallback repository for Windows platforms
                    fallback_repo = "{}_fallback_{}".format(attrs.name, python_platform)
                    python_interpreters[python_platform] = "@{}//file".format(fallback_repo)
                    python_integrities[python_platform] = ""
            ...
            for platform in scie_platforms:
                ...
                elif platform in windows_fallback_platforms:
                    fallback_repo = "{}_fallback_{}".format(attrs.name, platform)
                    pex_platforms_to_binaries[platform] = "@{}//file".format(fallback_repo)
                ...
                if platform in science_available:
                    ...
                    fallback_repo = "{}_fallback_{}".format(attrs.name, platform)
                    science_platforms_to_binaries[platform] = "@{}//file".format(fallback_repo)
                ...
                if platform in jump_available:
                    ...
                    fallback_repo = "{}_fallback_{}".format(attrs.name, platform)
                    jump_platforms_to_binaries[platform] = "@{}//file".format(fallback_repo)
                ...
                if platform in ptex_available:
                    ...
                    fallback_repo = "{}_fallback_{}".format(attrs.name, platform)
                    ptex_platforms_to_binaries[platform] = "@{}//file".format(fallback_repo)
...
_ADD_FALLBACK_WINDOWS_BINARIES_TAG = tag_class(
    doc = """\
Registers Windows fallback binaries for a specific toolchain.

This tag is used to add fallback binaries for Windows platforms when the actual
...
""",
    attrs = {
        "toolchain_name": attr.string(
            doc = "The name of the toolchain to add Windows fallback binaries for.",
            mandatory = True,
        ),
    },
)
...
pex = module_extension(
    ...
    tag_classes = {
        "add_fallback_windows_binaries": _ADD_FALLBACK_WINDOWS_BINARIES_TAG,
