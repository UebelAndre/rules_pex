...
load("//pex/private:pex_versions.bzl", _PEX_VERSIONS = "PEX_VERSIONS")
load("//pex/private:python_versions.bzl", _PYTHON_BUILD_STANDALONE_VERSIONS = "PYTHON_BUILD_STANDALONE_VERSIONS")
load("//pex/private:scie_jump_versions.bzl", _SCIE_JUMP_VERSIONS = "SCIE_JUMP_VERSIONS")
load("//pex/private:scie_ptex_versions.bzl", _SCIE_PTEX_VERSIONS = "SCIE_PTEX_VERSIONS")
load("//pex/private:scie_science_versions.bzl", _SCIE_SCIENCE_VERSIONS = "SCIE_SCIENCE_VERSIONS")
...
def _pex_windows_fallback_tool_repository_impl(repository_ctx):
    """Create a repository with an alias to the fallback binary."""
    repository_ctx.file("WORKSPACE.bazel", """workspace(name = "{}")""".format(
        repository_ctx.name,
    ))
...
echo pex is currently not compatible with windows. See https://github.com/UebelAndre/rules_pex/issues/1
exit 1
""",
    )
...
pex_windows_fallback_tool_repository = repository_rule(
    doc = "Creates a repository with an alias to the fallback binary for unsupported platforms (windows).",
    implementation = _pex_windows_fallback_tool_repository_impl,
)

_TOOLCHAIN_BUILD_TEMPLATE = """\
load("@rules_pex//pex:py_pex_toolchain.bzl", "py_pex_toolchain")
...
toolchain(
    ...
    toolchain_type = "@rules_pex//pex:toolchain_type",
