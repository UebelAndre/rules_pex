...
def verify_download(
    ...
    cmd = [str(science), "--cache-dir", str(cache_dir), "download"]
    ...
    # Set HOME and USERPROFILE to an empty temp directory for hermetic builds
    # This prevents science from accessing any user-specific configuration
    env = dict(os.environ)
    env["HOME"] = str(output_dir / "home")
    env["USERPROFILE"] = str(output_dir / "home")

    logging.debug("Verifying download with command: %s", " ".join(cmd))
    logging.debug("Using cache directory: %s", cache_dir)
    ...
    if result.returncode:
        ...
        raise RuntimeError(f"Failed to download: {target}")
...
def get_binary_version(binary_path: Path) -> str:
    ...
    with tempfile.TemporaryDirectory(
        dir=os.getenv("TEST_TMPDIR"), prefix="pex-bin-version-"
    ) as tmp_dir:

        env = dict(os.environ)
        env["HOME"] = str(tmp_dir)
        env["USERPROFILE"] = str(tmp_dir)
...
def main() -> None:
    ...
    is_debug = (
        "RULES_PEX_SCIE_CACHE_DEBUG" in os.environ or "RULES_PEX_DEBUG" in os.environ
    )
