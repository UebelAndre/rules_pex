...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the current workspace.",
    )
    parser.add_argument(
        "--pex",
        type=Path,
...
def main() -> None:  # pylint: disable=too-many-locals,too-many-statements
    ...
    is_debug = (
        "RULES_PEX_PROCESS_WRAPPER_DEBUG" in os.environ
        or "RULES_PEX_DEBUG" in os.environ
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        # rules_python doesn't include workspace in the `PyInfo.imports`
        # set for `py_binary` and thus workspace relative imports for
        # this package would fail.
        imports = args.imports
        workspace_name = args.workspace_name if args.workspace_name else "_main"
        if workspace_name not in imports:
            imports = [workspace_name] + imports
        ...
        venv_interpreter = create_venv(
            ...
            venv_dir=venv_dir,
            pth=[f"{runfiles_dir}/{i}" for i in imports],
        )
